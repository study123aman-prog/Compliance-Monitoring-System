...
## 1. Deliverable completeness (D1–D7, + D8 extension)
...
| D8 — Detection core | Extension | Complete | `docs/detection/detection-core.md` + `src/detection/`; two-tier detection (normalize → guardrails → agentic evaluator) + HITL routing |
...
All required documents from the D2 "Required Repository Structure" are present. The `src/` implementation and its tests are an addition beyond the required design docs, chosen to *prove* the design is internally consistent. The **D8 detection core** is a further additive enhancement: it makes each agent *compute* its opinion from raw evidence (rather than replay a fixed value) through an inspectable, deterministic pipeline, while reproducing all 20 locked outcomes exactly — so nothing in D1–D7 is changed or weakened.
...
| **Audit Completeness** | 157 hash-chained audit entries across the 20 cases (including per-opinion `detection.evaluated` verdicts + Chain-of-Thought and per-case HITL routing records); tamper-evidence demonstrated (detects a mutated entry, re-verifies after restore). |
...
- **Reference implementation is deliberately deterministic and ML-free.** The detection *layer* would use ML in production (described in the agent specs), but the coordination/consensus/escalation/audit layer is pure deterministic Python so it is explainable and reproducible. This is a considered design position (README §7), not a gap. The **D8 detection core** makes the ML boundary explicit and clean: its agentic evaluator scores through a pluggable backend, with a transparent deterministic scorecard as the default and an LLM backend that is deliberately inert unless a client is injected — so a model can be added to *detection* without ever touching the deterministic *decision* layer.
...
- `python -m src.run_demo` → **20/20** scenarios match locked outcomes; **157** audit entries; a worked detection-core example + the HITL review queue printed; tamper detected then restored; `ALL CHECKS PASSED`.
- Test suite → **99 tests pass** (23 scenario + 12 consensus + 8 audit + 56 detection).
- The 56 detection tests include a **reproduction suite**: every agent signal in every scenario is run through the detection pipeline and its *computed* severity/confidence/domain is asserted equal to the locked opinion — proving the "compute, don't replay" core cannot silently drift from the D6 outcomes.
